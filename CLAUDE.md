...
|----------|---|-----------|---------|-----------------|------|
| `sensitive` | 50 | seeded WFA, all chain anchors | top 50 chains (+ (0,0) fallback only if no chains) | ±25bp | canonical reference path |
| `balanced` (default) | 2 | Myers fitting ≤500bp, WFA fallback | top 2 chain anchors (no fallback when chains exist) | ±50bp | recall-balanced, faster engine |
| `fast` | 1 | Myers/WFA + divergence cutoff | single best chain anchor (no fallback when chains exist) | ±150bp | low-recall survey |

- The chain cap K controls recall and ambiguity preservation: `sensitive` (K=50) extends many chains for thorough multi-mapping, `balanced` (K=2) extends the top 2 chains (primary + best alternative), `fast` (K=1) extends only the best. All strategies use seed chaining (ADR-0012) to collapse raw minimizer seeds into co-linear candidate loci before extension. The `(0,0)` fallback anchor is only included when chaining finds zero chains — the safety net for low-complexity/repetitive backgrounds (issue #146) where chaining can't distinguish signal from noise.
- `fast` trades recall for speed: K=1 chain capping collapses per-read anchor count to O(1) in repeats (under-reports multi-mapping), and reads whose best alignment exceeds `FAST_MAX_DIVERGENCE` (0.20, hard-coded opinion) are dropped.
- Myers/WFA both run in **fitting** mode (query fully consumed, target end free) above the `n ≤ m + m/2 + 10` threshold; below it both fall back to global, so they stay consistent. In fitting mode, early termination abandons the forward pass once the edit-distance score rises more than `m/10` above the best seen so far.
- **ADR-0007 branch-and-bound is retired** (superseded by ADR-0012 chaining). The old mechanism (`score_bound_max_s`, `wfa_extend_capped`) was removed — chaining structurally obsoletes the anchor-explosion problem it mitigated.
- Note: `.phraya`/`.phraya.queries` output is byte-deterministic run-to-run (all HashMaps serialize via sorted BTreeMap at the write boundary). The only non-deterministic field is the header timestamp, pinnable with `PHRAYA_SOURCE_DATE`.
