...
impl ThresholdFilter {
    ...
    pub fn apply(&self, obs: &VariantObservation) -> bool {
        ...
        // Filter 4: Both mates mapped — uses unmapped_mate_fraction (merge-stable).
        // A position is excluded if >50% of its covering paired reads have an unmapped mate.
        // Falls back to mate_info for the pre-merge single-read case.
        if self.require_both_mates_mapped {
            match obs.unmapped_mate_fraction() {
                Some(frac) if frac > 0.5 => return false,
                None => {
                    // No aggregate data: fall back to mate_info (pre-merge unpaired reads).
                    if let Some(mate_info) = obs.mate_info() {
                        if !mate_info.mate_mapped {
                            return false;
                        }
                    }
                    // No mate_info and no aggregate data → unpaired read, pass through
                    // (require_both_mates_mapped is not applicable to unpaired reads)
                }
                _ => {}
            }
        }
